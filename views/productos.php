  ...
  <div class="content">
    ...
    <div class="card">
      ...
      <div class="table-wrap">
        <table id="tbl">
          <thead>
            <tr><th>Foto</th><th>#</th><th>Nombre</th><th>Categoría</th><th>Precio</th><th>Stock</th><th>Tallas</th><th>Estado</th><?php if(isAdmin()):?><th>Acciones</th><?php endif;?></tr>
          </thead>
          <tbody>
              ...
              <td>
                ...
                <?php else: ?>
                  <div style="display:flex;flex-wrap:wrap;gap:4px;max-width:180px">
                    ...
                    <span title="<?=$t['stock']?> uds" style="font-size:.68rem;padding:2px 7px;border-radius:12px;font-weight:600;border:1px solid;<?=$t['stock']==0?'border-color:var(--danger);color:var(--danger);background:var(--danger-dim)':($t['stock']<=3?'border-color:var(--warning);color:var(--warning);background:var(--warning-dim)':'border-color:var(--gold);color:var(--gold-light);background:var(--gold-dim)')?>">
  ...
  <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:12px;width:100%;max-width:580px;max-height:90vh;overflow-y:auto">
    ...
    <div style="padding:24px">
      ...
      <div style="background:var(--gold-dim);border:1px solid rgba(201,168,76,0.2);border-radius:8px;padding:12px 14px;margin-bottom:20px;font-size:.82rem;color:var(--white-dim);line-height:1.6">
        💡 Agrega cada talla con su cantidad. Puedes usar tallas de ropa (XS, S, M, L, XL, XXL) o numéricas (28, 30, 32...) o cualquier denominación.
      </div>
      ...
      <div style="background:var(--bg-panel);border-radius:10px;padding:16px;margin-bottom:20px;border:1px solid var(--border)">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr auto;gap:10px;align-items:end">
          <div class="form-group" style="margin:0">
            <label>Talla</label>
            <input type="text" id="nueva-talla" placeholder="Ej: M, XL, 32..." style="text-transform:uppercase">
          </div>
          <div class="form-group" style="margin:0">
            <label>Cantidad</label>
            <input type="number" id="nueva-cantidad" placeholder="Ej: 10" min="0" step="1" value="0">
          </div>
          <button class="btn btn-primary" onclick="guardarTalla()">Agregar</button>
        </div>
        <!-- Accesos rápidos de tallas comunes -->
      ...
      <div id="tallas-lista">
        <div style="text-align:center;padding:30px;color:var(--white-muted);font-size:.85rem">Cargando tallas...</div>
      </div>
    </div>
  </div>
</div>
...
function borrar(btn) {
  ...
  const nm = td.querySelector('.d-nombre').textContent.trim();
...
function previewImg(input) {
  ...
  new FileReader().onload = e => mostrarPrev(e.target.result, file.name);
...
async function cargarTallas(prodId) {
  ...
  } catch(e) { document.getElementById('tallas-lista').innerHTML='<div style="color:var(--danger);padding:20px;text-align:center">Error al cargar tallas</div>'; }
}

function renderTallas(tallas) {
  const cont = document.getElementById('tallas-lista');
  if (!tallas.length) {
    cont.innerHTML = '<div style="text-align:center;padding:30px;color:var(--white-muted);font-size:.85rem">📭 No hay tallas registradas aún</div>';
    return;
  }
  const total = tallas.reduce((a,t)=>a+parseInt(t.stock),0);
  cont.innerHTML = `
    <div style="background:var(--bg-panel);border-radius:8px;overflow:hidden;border:1px solid var(--border)">
      <table style="width:100%;border-collapse:collapse">
        <thead>
          <tr>
            ...
            <th style="padding:10px 14px;text-align:center;font-size:.7rem;text-transform:uppercase;letter-spacing:.1em;color:var(--white-muted);border-bottom:1px solid var(--border)">Stock</th>
            <th style="padding:10px 14px;text-align:center;font-size:.7rem;text-transform:uppercase;letter-spacing:.1em;color:var(--white-muted);border-bottom:1px solid var(--border)">Editar</th>
        ...
        <tbody>
          ...
          <tr style="border-bottom:1px solid rgba(255,255,255,0.04)">
            <td style="padding:11px 14px">
              <span style="font-size:1rem;font-weight:700;color:var(--white);font-family:var(--font-display)">${t.talla}</span>
            </td>
            ...
            <td style="padding:11px 14px;text-align:center">
              <input type="number" min="0" value="${t.stock}" style="width:70px;text-align:center;padding:5px 8px;background:var(--bg-hover);border:1px solid var(--border);border-radius:6px;color:var(--white);font-size:.85rem"
                onchange="actualizarStock('${t.talla}', this.value)" id="stock-input-${t.id}">
            </td>
            <td style="padding:11px 14px">
        ...
        <tfoot>
          <tr>
            <td style="padding:10px 14px;font-size:.78rem;color:var(--white-muted);font-weight:600">TOTAL</td>
            <td style="padding:10px 14px;text-align:center;font-weight:700;color:var(--gold-light)">${total} uds</td>
            <td colspan="3"></td>
          </tr>
        </tfoot>
      </table>
...
async function guardarTalla() {
  ...
  fd.append('action','guardar'); fd.append('producto_id',prodId);
  fd.append('talla',talla); fd.append('stock',cantidad);
...
async function actualizarStock(talla, nuevoStock) {
  ...
  const fd = new FormData();
  fd.append('action','guardar'); fd.append('producto_id',prodId);
  fd.append('talla',talla); fd.append('stock',parseInt(nuevoStock)||0);
  const r = await fetch(CTRL_TALLA,{method:'POST',body:fd});
  const d = await r.json();
