...
function borrar(btn){
  ...
  fetch('<?=BASE_URL?>/controllers/DescuentoController.php', {
    method: 'POST',
    body: fd
  })
  .then(response => {
...
document.getElementById('form-desc').addEventListener('submit', async function(e) {
  e.preventDefault();

  const formData = new FormData(this);

  try {
    const response = await fetch('<?=BASE_URL?>/controllers/DescuentoController.php', {
      method: 'POST',
      body: formData
    });
    ...
    if (data.success) {
      ...
      alert('Error: ' + (data.error || 'No se pudo guardar el descuento'));
    }
  } catch (error) {
    console.error('Error:', error);
    alert('Error de conexión: ' + error.message + '. Intenta de nuevo.');
