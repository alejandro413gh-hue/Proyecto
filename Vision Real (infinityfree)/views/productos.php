...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && puedeGestionarInventario()) {
    ...
    if ($action === 'delete' && $id > 0) {
        ...
        elseif ($precio <= 0) { $error = 'El precio debe ser mayor a 0'; }
            ...
            if (!$error) {
                if ($action === 'create') {
                    $r = $pm->create($nombre, $desc, $precio, $stock, $cat, $imagen_nueva);
                    ...
                    $r = $pm->update($id, $nombre, $desc, $precio, $stock, $cat, $imagen_nueva);
...
<div class="main">
  <header class="topbar">
    ...
    <div class="topbar-right">
      <a href="<?= BASE_URL ?>/views/productos_export.php" class="btn btn-outline btn-sm">⬇️ Exportar Excel</a>
    ...
    <div class="card" style="margin-bottom:20px">
      ...
      <div style="padding:16px 20px">
        ...
        <div id="no-encontrado" style="display:none;margin-top:16px;padding:14px 16px;background:var(--bg-panel);border-radius:10px;border:1px solid rgba(231,76,60,.3)">
          ...
          <?php if(puedeGestionarInventario()): ?>
          <button class="btn btn-primary btn-sm" style="margin-top:10px" onclick="crearConCodigo()">+ Crear producto nuevo</button>
          <?php endif; ?>
        </div>
      </div>
    </div>
      ...
      <div class="form-grid">
        ...
        <div class="form-group span-2">
          ...
          <div style="display:flex;gap:8px;align-items:center">
            <input type="text" name="codigo" id="f-codigo" placeholder="Generado automaticamente" style="text-transform:uppercase;flex:1" readonly>
...
// ===== PRODUCTO MODAL =====
function abrirNuevo(codigoInicial) {
  codigoInicial = (codigoInicial || '').trim().toUpperCase();
  ...
  if (codigoInicial) {
    document.getElementById('f-codigo').value = codigoInicial;
    var fb = document.getElementById('codigo-feedback');
    fb.textContent = 'Código prellenado desde la búsqueda';
    fb.style.color = 'var(--success)';
  } else {
    generarCodigo();
  }
...
function renderTallas(tallas){
  ...
  cont.innerHTML='<div style="background:var(--bg-panel);border-radius:8px;overflow:hidden;border:1px solid var(--border)"><table style="width:100%;border-collapse:collapse"><thead><tr>'+
    ...
    tallas.map(function(t){return '<tr style="border-bottom:1px solid rgba(255,255,255,0.04)">'+
      ...
      '<td style="padding:11px 14px;text-align:center"><input type="number" min="0" value="'+t.stock+'" style="width:70px;text-align:center;padding:5px 8px;background:var(--bg-hover);border:1px solid var(--border);border-radius:6px;color:var(--white);font-size:.85rem" onchange="actualizarStock('+t.id+', this.value)"></td>' +
...
async function guardarTalla(){
  ...
  var cantidad=parseInt(document.getElementById('nueva-cantidad').value);
  ...
  if(Number.isNaN(cantidad) || cantidad < 0){errEl.textContent='Cantidad inválida';errEl.style.display='block';return;}
...
async function actualizarStock(id,nuevoStock){
  var prodId=document.getElementById('talla-prod-id').value;
  var cantidad=parseInt(nuevoStock);
  if(Number.isNaN(cantidad) || cantidad < 0) return;
  var fd=new FormData();fd.append('action','actualizar');fd.append('id',id);fd.append('producto_id',prodId);fd.append('stock',cantidad);
